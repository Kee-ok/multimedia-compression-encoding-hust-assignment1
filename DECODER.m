...
function synth_speech = DECODER (aCoeff, pitch_plot, voiced, gain);
...
for b = 1 : frame_length : (length(gain)),    %length(gain) should be very close 
    ...
    if voiced(b) == 1,   %voiced frame
        ...
        syn_y1 = SYN_VOICED(aCoeff, gain, frame_length, pitch_plot_b, b);
    else
        syn_y1 = SYN_UNVOICED(aCoeff, gain, frame_length, b); %unvoiced frame
    end
